...
public abstract class Riemann {
	public double rs(polyfun.Polynomial poly, double left, double right, int subintervals) {
		...
		for (double ii = 0; ii < subintervals; ii ++) {
			area += this.slice(poly, left, left+delta); //this method is implemented by each rule class 
	...
	public void rsAcc(org.opensourcephysics.frames.PlotFrame pframe, polyfun.Polynomial poly, int index, double precision, double base) {
		...
		for (double ii = base-x; ii <= base+x; ii += precision) {
			...
			//double curRs = this.rs(poly, base, ii, (int) ((int)Math.abs(base-ii)/ii)); //current riemann sum - uses 250 
